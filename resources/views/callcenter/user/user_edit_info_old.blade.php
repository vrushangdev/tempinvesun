                ...
                <div class="card m-b-30 addLeadAssistant">
                    <div class="card-body">
                        ...
                        <div class="form-group reschedule_date"  @if(is_null($getUserInfo) && $getUserInfo->reschedule_date == '') style="display:none;" @endif>
                            ...
                            <input type="text" class="form-control js-datepicker" id="inputAppoDate" placeholder="Appointment Date" name="reschedule_date" value=" @if(!is_null($getUserInfo) && $getUserInfo->reschedule_date != '') {{ $getUserInfo->reschedule_date }} @else {{ date('d/m/Y') }} @endif" autocomplete="off" required>
                        </div>
...
<script type="text/javascript">
    $(document).on('change','#status',function(){
        if($(this).val() == 1){
            $('.reschedule_date').show();
            $('.reschedule_time').show();
        } else {
            $('.reschedule_date').hide();
            $('.reschedule_time').hide();
        }
    });
